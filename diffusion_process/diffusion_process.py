...
class DiffusionProcess(nn.Module):
    ...
    def sample(self, x_0: Tensor, t: LongTensor) -> Tuple[Tensor, Tensor]:
        ...
        return scaled_x_0 + noise, noise

        # epsilon_0_to_t = self.noise_distribution.sample((x_0.shape[0],)).reshape(x_0.shape).to(x_0.device)
        # noise_0_to_t = epsilon_0_to_t * (1 - self.alpha_bar_t[t - 1]).sqrt().unsqueeze(-1).unsqueeze(-1) 
