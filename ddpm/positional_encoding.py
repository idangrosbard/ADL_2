...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor, t: Tensor) -> Tensor:
        ...
        pe[:, 0::2] = torch.sin(t / (2 * self.max_T) ** (torch.arange(0, self.d_model ** 2, 2).float() / self.d_model ** 2))
        # cosine for odd indices
        pe[:, 1::2] = torch.cos(t / (2 * self.max_T) ** (torch.arange(1, self.d_model ** 2, 2).float() / self.d_model ** 2))
