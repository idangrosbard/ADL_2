...
class Trainer(object):
    ...
    def epoch(self, data_loader: DataLoader, train: bool = True) -> float:
        ...
        for x_0, t in pbar:
            ...
            if train:
                self.summary_writer.add_scalar(f'LR', self.scheduler.get_lr(), self.total_steps)
            if (self.total_steps % self.sampling_freq) == 0:
                with torch.no_grad():
                    self.samplers.eval()
                    for sampler in self.samplers:
                        sample = sampler(self.n_samples)
                        for i in range(self.n_samples):
                            self.summary_writer.add_image(f'{type(sampler)} sampled image', self.image_denormalize(sample[i]), self.total_steps)
                    self.samplers.train()
